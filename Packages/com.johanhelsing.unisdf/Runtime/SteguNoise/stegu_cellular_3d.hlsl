#ifndef JGJK_STEGU_CELLULAR_3D_HLSL
#define JGJK_STEGU_CELLULAR_3D_HLSL
...
float2 stegu_cellular_3d_f1f2(float3 P)
{
...
// a lot slower than the 2x2x2 version.
// The code below is a bit scary even to its author,
// but it has at least half decent performance on a
// modern GPU. In any case, it beats any software
// implementation of Worley noise hands down.
float2 stegu_cellular_3d_f1(float3 P)
{
#define K 0.142857142857 // 1/7
#define Ko 0.428571428571 // 1/2-K/2
#define K2 0.020408163265306 // 1/(7*7)
...
#define jitter 1.0 // smaller jitter gives more regular pattern
    ...
    return sqrt(d.x); // no F2 computed
}
// Shader graph interface functions

void stegu_cellular_3d_f1_float(float3 p, out float distance)
{
    distance = stegu_cellular_3d_f1(p);
}

void stegu_cellular_3d_f1f2_float(float3 p, out float2 distances)
{
    distances = stegu_cellular_3d_f1f2(p);
}
